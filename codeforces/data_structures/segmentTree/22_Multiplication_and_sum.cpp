...
struct node{
    ...
    bool clazy = 0;
    node(){};
    node(ll s, ll p, ll c): sum(s), prod(p), clazy(c) {};
};
...
public:
    ...
    node merge(node &l, node &r){
        node cur;
        cur.sum = add(mul(l.sum, l.prod), mul(r.sum, r.prod));
        cur.prod = 1;
        cur.clazy = 0;
        return cur;
    }
    ...
    void propagate(int pos, int sl, int sr){
        ...
        if(sl!=sr){
            mul_self(st[2*pos].prod, st[pos].prod);
            mul_self(st[2*pos+1].prod, st[pos].prod);
            st[2*pos].clazy = st[2*pos+1].clazy = 1;
        }
        mul_self(st[pos].sum, st[pos].prod);
        st[pos].prod = 1;
        st[pos].clazy = 0;
    }
