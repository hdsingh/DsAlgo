...
struct node{
    ll mx = 0, pref = 0, suf = 0, sum = 0;
    node(){};
    node(ll val): mx(val), pref(val), suf(val), sum(val) {};
    node(ll _mx, ll _pref, ll _suf, ll _sum): mx(_mx), pref(_pref), suf(_suf), sum(_sum) {};
};
...
public:
    ...
    node merge(node& l, node &r){
        node cur;
        cur.mx = max({0ll, l.suf + r.pref, l.mx, r.mx});
        cur.pref = max({0ll, l.pref, l.sum + r.pref});
        cur.suf = max({0ll, r.suf, r.sum + l.suf});
        cur.sum = l.sum + r.sum;
        return cur;
    }
