...
struct node{
    ...
    bool clazy = 0;
    node(){};
    node(int a, int o, bool c):  And(a), Or(o), clazy(c) {};
};
...
public:
    ...
    void propagate(int pos, int sl, int sr){
        ...
        if(sl!=sr){
            st[2*pos].Or|=st[pos].Or;
            st[2*pos+1].Or|=st[pos].Or;
            st[2*pos].clazy = st[2*pos+1].clazy = 1;
        }
        st[pos].And |= st[pos].Or;
        st[pos].Or = st[pos].clazy = 0;
    }
