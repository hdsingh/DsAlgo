...
struct node{
    ll sum = 0, add = 0, len = 0;
    bool clazy = 0;
    node(){};
    node(ll s, ll a, ll l): sum(s), add(a), len(l) {};
};
...
public:
    ...
    node merge(node &l, node &r){
        node cur;
        cur.sum = (l.sum + (l.len*l.add)) + (r.sum + (r.len*r.add));
        cur.len = l.len + r.len;
        return cur;
    }
    ...
    void propagate(int pos, int sl, int sr){
        ...
        if(sl!=sr){
            st[2*pos].add+=st[pos].add;
            st[2*pos+1].add+=st[pos].add;
            st[2*pos].clazy = st[2*pos+1].clazy = 1;
        }
        st[pos].sum+=st[pos].add*st[pos].len;
        st[pos].add = st[pos].clazy = 0;
    }
