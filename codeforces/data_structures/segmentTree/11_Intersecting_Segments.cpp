...
int main(){
    ...
    forn(i,2*n){
        if(!seen.count(a[i])){
            ...
            int len = i - seen[a[i]] - 1;
            int val = st.query(1,0, 2*n-1, seen[a[i]], i);
            ans[a[i]] = len - 2*val;
            st.update(1, 0, 2*n-1, seen[a[i]]);
        }
    }
...
// Extending the prev solution:
// If we know completely nested segments inside, we can simply calc
// the intersecting segs by (# of elements in (l to r)) - 2*nested