...
void dfs(int x, int p, int gc, set<int> skip){
    int mx = gc; // by skipping this
    int new_gc = __gcd(gc,a[x]);
    mx = max(mx,new_gc); // not skipping

    set<int> new_skip = {gc}; // skip this
    // since prev skipped, so include this
    for(auto s: skip){
        new_skip.insert(__gcd(s,a[x]));
    }
    mx = max(mx, *new_skip.rbegin());
    ans[x] = mx;
    for(auto ad: adj[x]){
        if(ad==p) continue;
        dfs(ad,x,new_gc,new_skip);
    }
}
