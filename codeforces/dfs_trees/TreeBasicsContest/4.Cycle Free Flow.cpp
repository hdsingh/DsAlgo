...
int main(){
    ...
    int n, m;
    while(cin>>n>>m){
        vector<vector<pii>> adj(n+1); // {ad, wt}
        forn(i,m){
            int x, y, w; cin>>x>>y>>w;
        ...
        function<void(int,int,int,int)> dfs = [&](int node, int p, int lev, int w){
            tin[node] = tim++;
            par[0][node] = p;
            wt[0][node] = w;
            level[node] = lev;

            for(auto [ad, adw]: adj[node]){
                if(ad==p) continue;
                dfs(ad, node,lev+1, adw);
            }
            tout[node] = tim++;
        };
        ...
        fore(j,1,LG){
            fore(i,1,n+1){
                if(par[j-1][i]){
                    ...
                    wt[j][i] = min(wt[j-1][i], wt[j-1][par[j-1][i]]);
                }
            }
        }
        ...
        while(q--){
            ...
            if(isAncestor(a,b) && level[a]<=level[b]){
                ...
                cout<<min(walk(a,alc).y, walk(b,lcb).y)<<"\n";
            }
            // cout<<"\n";
        }
    }
