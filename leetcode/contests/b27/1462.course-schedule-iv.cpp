...
// reachability
class Solution {
    vvi reachable; // is i reachable from j
    vvi adj;
    int n;
public:
    vector<bool> checkIfPrerequisite(int N, vector<vector<int>>& preqs, vector<vector<int>>& queries) {
        n = N;
        reachable.clear();
        reachable.resize(n, vi(n));
        ...
        vector<bool> out;
        for(auto q: queries){
            out.push_back(reachable[q[1]][q[0]]);
        }
...
int main(){
    int n; vvi prerequisites, queries;
    Solution sol; vb out;
