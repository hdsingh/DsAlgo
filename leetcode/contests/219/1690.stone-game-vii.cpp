...
public:
    ...
    int take(int now, int lt, int rt){
        return getSum(lt,rt) - a[now];
    }
    ...
    int dfs(int lt, int rt){
        ...
        int op1 = take(lt,lt,rt) - dfs(lt+1,rt);
        int op2 = take(rt,lt,rt) - dfs(lt,rt-1);
        ans = max(op1,op2);
        return ans;
    }
};
...
int main(){
    ...
    vi a; int out;
    a = { 5,3,1,4,2 };
    out = sol.stoneGameVII(a); deb(out);
